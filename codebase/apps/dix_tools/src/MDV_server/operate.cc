     ...
     
{
  ...
  if (Glob->params.projection == PJG_LATLON) {
      reply->divisor = (int) CDATA_LATLON_DIVISOR;
  } else {
...

{
  ...
  if (Glob->params.projection == PJG_LATLON) {
      info->divisor = (int) CDATA_LATLON_DIVISOR;
  } else {
  ...
  info->nyquist_vel = (int) floor
    (rparams->prf * (rparams->wavelength / 100.0) *
     info->divisor / 4.0 + 0.5);
