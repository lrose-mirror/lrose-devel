...
typedef	struct {
    ...
    uint16_t	size;		/*  Message size in halfwords for
    ...
    uint16_t	sequence_num;	/*  Message Sequence Number from 0 - 65535
					    then to 0
					*/

    uint16_t	julian_date;	/*  Julian date from January 1, 1970 at
					    0000 UT (starting with 1)
					*/

    uint32_t	milliseconds; 	/*  Number of milliseconds past midnight (UT)
					*/ 

    uint16_t	num_segs;	/*  Number of message segments.  Messages
    ...
    uint16_t	seg_num;	/*  Segment number of this message
					*/
