...
#include <cstring>
...

  {
    ...
    tt->comment_hdr = tdrpStrDup("DsrWebPost reads radar beam-by-beam data from a DSR-format FMQ and posts the data to the specified URL.");
    ...
    tt->struct_def.fields = (struct_field_t *)
      ...
      tt->struct_def.fields[0].rel_offset = 
        (char *) &radar_location.name - (char *) &radar_location;
    ...
    // Parameter 'output_raw_data'
    // ctype is 'tdrp_bool_t'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = BOOL_TYPE;
    tt->param_name = tdrpStrDup("output_raw_data");
    tt->descr = tdrpStrDup("Option to output data files on disk.");
    tt->help = tdrpStrDup("If TRUE, raw data files will be output at the location specified in 'output_raw_dir'.");
    tt->val_offset = (char *) &output_raw_data - &_start_;
    tt->single_val.b = pFALSE;
    tt++;
    
    // Parameter 'output_raw_dir'
    // ctype is 'char*'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = STRING_TYPE;
    tt->param_name = tdrpStrDup("output_raw_dir");
    tt->descr = tdrpStrDup("Dir of raw data output files.");
    tt->help = tdrpStrDup("This only needs to be set if 'output_raw_data' is TRUE. The directory must exist.");
    tt->val_offset = (char *) &output_raw_dir - &_start_;
    tt->single_val.s = tdrpStrDup("./output");
    tt++;
    
    // Parameter 'output_raw_prefix'
    // ctype is 'char*'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = STRING_TYPE;
    tt->param_name = tdrpStrDup("output_raw_prefix");
    tt->descr = tdrpStrDup("Prefix to the file name.");
    tt->help = tdrpStrDup("This only needs to be set if 'output_raw_data' is TRUE.");
    tt->val_offset = (char *) &output_raw_prefix - &_start_;
    tt->single_val.s = tdrpStrDup("Radar_");
    tt++;
    
    // Parameter 'output_raw_ext'
    // ctype is 'char*'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = STRING_TYPE;
    tt->param_name = tdrpStrDup("output_raw_ext");
    tt->descr = tdrpStrDup("Extension to the file name.");
    tt->help = tdrpStrDup("This only needs to be set if 'output_raw_data' is TRUE.");
    tt->val_offset = (char *) &output_raw_ext - &_start_;
    tt->single_val.s = tdrpStrDup("raw");
    tt++;
    
    // Parameter 'radial_collection_size'
    // ctype is 'long'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = LONG_TYPE;
    tt->param_name = tdrpStrDup("radial_collection_size");
    tt->descr = tdrpStrDup("The number of radials to collect in a data data file.");
    tt->help = tdrpStrDup("Should be set to 120 for bandwidth purposes.");
    tt->val_offset = (char *) &radial_collection_size - &_start_;
    tt->single_val.l = 120;
    tt++;
    
    // Parameter 'override_vcp'
    // ctype is 'tdrp_bool_t'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = BOOL_TYPE;
    tt->param_name = tdrpStrDup("override_vcp");
    tt->descr = tdrpStrDup("Option to override the VCP number.");
    tt->help = tdrpStrDup("If TRUE, the application will use the specified VCP number");
    tt->val_offset = (char *) &override_vcp - &_start_;
    tt->single_val.b = pFALSE;
    tt++;
    ...
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = LONG_TYPE;
    tt->param_name = tdrpStrDup("vcp");
    tt->descr = tdrpStrDup("Use this VCP for the collected data.");
    tt->help = tdrpStrDup("Only used if 'override_vcp' is TRUE.");
    tt->val_offset = (char *) &vcp - &_start_;
    tt->single_val.l = 21;
    tt++;
    
    // Parameter 'tilt_increment'
    // ctype is 'tdrp_bool_t'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = BOOL_TYPE;
    tt->param_name = tdrpStrDup("tilt_increment");
    tt->descr = tdrpStrDup("Option to increment the tilt number. This is sometimes needed for NEXRAD data sources to avoid 0-based tilt numbers.");
    tt->help = tdrpStrDup("If TRUE, the application will increment the tilt number by 1.");
    tt->val_offset = (char *) &tilt_increment - &_start_;
    tt->single_val.b = pFALSE;
    tt++;
    
    // Parameter 'post_data_web'
    // ctype is 'tdrp_bool_t'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = BOOL_TYPE;
    tt->param_name = tdrpStrDup("post_data_web");
    tt->descr = tdrpStrDup("Option to post data to a web server in JSON format.");
    tt->help = tdrpStrDup("If TRUE, data will be posted to the URL specified in 'post_data_url'.");
    tt->val_offset = (char *) &post_data_web - &_start_;
    tt->single_val.b = pFALSE;
    tt++;
    
    // Parameter 'post_data_url'
    // ctype is 'char*'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = STRING_TYPE;
    tt->param_name = tdrpStrDup("post_data_url");
    tt->descr = tdrpStrDup("URL to post data in JSON format.");
    tt->help = tdrpStrDup("This only needs to be set if 'post_data_web' is TRUE.");
    tt->val_offset = (char *) &post_data_url - &_start_;
    tt->single_val.s = tdrpStrDup("http://localhost/PostRadarDataJSON");
    tt++;
