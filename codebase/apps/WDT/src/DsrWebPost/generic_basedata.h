 ...
 * $Author: steves $
 * $Locker:  $
 * $Date: 2007/03/21 14:37:13 $
 * $Id: generic_basedata.h,v 1.10 2007/03/21 14:37:13 steves Exp $
 * $Revision: 1.10 $
 * $State: Exp $
...
typedef struct {
    ...
    uint16_t len;                 /* Length of this record, in bytes. */
    ...
    int16_t height;                       /* Radar height, in feet MSL. */

    uint16_t feedhorn_height;     /* Height of feedhorn, in feet AGL. */
    ...
    uint16_t vcp_num;             /* Volume coverage pattern. */

    int16_t spare;			/* Needed to force 4 byte alignment. */
...
typedef struct {
    ...
    int16_t atmos;                        /* Atmospheric attenuation factor, in dB/km*1000. */
...
typedef struct {
    ...
    uint16_t unamb_range;         /* Unambiguous range, in km*10. */

    float horiz_noise;                  /* Horizontal Noise, in dBm. */

    float vert_noise;                   /* Vertical Noise, in dBm. */

    uint16_t nyquist_vel;         /* Nyquist velocity, in m/s*100. */
...
typedef struct {
    ...
    uint32_t info;			/* An offset to moment specific information for this
			   		   moment. */

    uint16_t no_of_gates;		/* Number of gates for this moment. */

    int16_t first_gate_range;		/* Range to the center of the first gate, in m. */

    int16_t bin_size;			/* Size of each gate for this moment in m. */

    uint16_t tover;		/* Threshold parameter which specifies the minimum
				   	   difference in echo power between two resolution
				   	   gates for them not to be labeled "overlayed", in dB*10. */

    int16_t SNR_threshold;		/* SNR threshold for valid data, in dB*8. */
    ...
    union {				 /* Variable length array of moment data. "no_of_gates"
        ...
        uint16_t u_s[0];		/* Use this if data_word_size is 16. */

        uint32_t u_i[0];		/* Use this if data_word_size is 32 and scale 
					   is not 0.0.  */
...
typedef struct{
   ...
   int16_t unique_param_size;		/* Size, in bytes, for this moments unique parameters. */
...
typedef struct{
    ...
    uint32_t time;			/* Collection time for this radial in
					   millisecs of day past midnight
					   (GMT). */

    uint16_t date;		/* Modified Julian date from 1/1/70. */

    int16_t azi_num;			/* Radial number within elevation
					   scan. */
    ...
    int16_t radial_length;		/* Uncompressed length of the radial
    ...
    uint16_t no_of_datum;		/* Number of data items in this
					   radial. */

    uint32_t data[0];			/* Variable length array of data
