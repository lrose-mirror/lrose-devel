 ...
 * $Author: steves $
 * $Locker:  $
 * $Date: 2011/02/17 21:43:54 $
 * $Id: basedata.h,v 1.77 2011/02/17 21:43:54 steves Exp $
 * $Revision: 1.77 $
 * $State: Exp $
 * $Log: basedata.h,v $
 * Revision 1.77  2011/02/17 21:43:54  steves
 * issue 3-844
 *
...
typedef struct nx_msg_hdr {
  int16_t msg_len;		/* Message size in halfwords measured from 
			   	 * this halfword to end of record BDMSIZ=1 */

  int16_t msg_type;		/*  Lower byte  (Regardless of endianess of the machine)  
  ...
  int16_t seq_num; 		/* I.D. sequence = 0 to 7fff, then to 0. 
				 * BDIDSEQ=3 */

  int16_t msg_date;		/* Modified julian date starting from * 1/1/70 
				 * BDJDAT=4 */ 

  uint32_t msg_time;		/* Generation time of messages in millisecs of 
			   	 * day past midnight (GMT) BDMILTIM=5.  
		    	 	 * This may be different than data time bellow */

  int16_t num_msg_segs;		/* Number of message segments. BDTOTSEG=7 */

  int16_t msg_seg_num;		/* Message segment number BDSEGNUM=8 */

  uint32_t time;			/* Collection time for this radial in millisecs 
			    	 * of day past midnight (GMT). BDZULU=9 */

  int16_t date;           	/* Modified julian date from 1/1/70 BDCDAT=11 */

  int16_t unamb_range;    	/* Unambiguous range (scaled: val/10 = KM) 
			   	   BDUNAMBI=12 */

  uint16_t azimuth;       /* Azimuth angle BDAZA=13
  ...
  int16_t azi_num;		/* Radial number within elevation scan (1, 2, ...)
			   	 * BDAZN=14 */

  int16_t status;			/* Radial status where: BDRSTAT=15
  ...
  uint16_t elevation;     /* Elevation angle  BDEANG=16
			         * (coded: (val/8)*(180/4096) = DEG). An elevation
			         * of '0 degs' is parallel to the pedestal base
			         * while '90 degs' is perpendicular to the
			         * pedestal base. */

  int16_t elev_num;       	/* RDA elevation number within VCP (1, 2, ...)
			   	 * scan BDENUM=17 */

  int16_t surv_range;		/* Range to first gate of reflectivity data
			   	 * METERS BDSRANG=18 */

  int16_t dop_range;      	/* Range to first gate of Doppler data.
			   	 * Doppler data - velocity and spectrum width
			   	 * METERS BDDRANG=19 */

  int16_t surv_bin_size;		/* Reflectivity data gate size METERS 
			   	 * BDSINT=20 */

  int16_t dop_bin_size;		/* Doppler data gate size METERS BDDINT=21 */ 

  int16_t n_surv_bins;		/* Number of reflectivity gates BDNSB=22;
			   	 * n_bins' reduce when elevation increases; */

  int16_t n_dop_bins;		/* Number of velocity and/or spectrum width
			   	 * data gates BDNDB=23; 
			         * n_bins = 0 if the field is missing; */

  int16_t sector_num;	 	/* Sector number within cut (1 - 3) BDSPP=24; */

  float calib_const;		/* System gain calibration constant. */

  int16_t ref_ptr;		/* Reflectivity data pointer (byte # from
				 *  start of digital radar message header.)
			 	 *  Set to 0 if reflectivity moment disabled. 
                         	 *  BDSP=27. */

  int16_t vel_ptr;		 /* Velocity data pointer (byte # from
  ...
  int16_t spw_ptr;		 /* Spectrum width pointer (byte # from
			 	  * start of digital radar message header). This
				  * pointer locates the beginning of spectrum 
				  * width data. Set to 0 is spectrum width moment
        	                  * disabled. BDSWP=29 */

  int16_t vel_resolution;		 /* Doppler velocity resolution BDDVR=30
				  * 2 = 0.5 m/s
				  * 4 = 1.0 m/s */

  int16_t vcp_num; 	  	 /* Volume coverage pattern BDVCP=31. */

  int16_t word_32;	 	 /* unused. */
  int16_t word_33;	 	 /* unused. */
  int16_t word_34;	 	 /* unused. */
  int16_t word_35;		 /* unused */

  int16_t ref_data_playback;	 /* Reflectivity data pointer for
                           	    Archive II playback */

  int16_t dop_data_playback;	 /* Velocity data pointer for
                           	    Archive II playback */

  int16_t sw_data_playback;	 /* Spectrum width data pointer for
                           	    Archive II playback */

  int16_t nyquist_vel;		 /* Nyquist velocity BDNQVEL=39 (scaled: 
                                  * val/100 = m/s) */

  int16_t atmos_atten;		 /* Atmospheric attenuation factor. 
			          * BDATMOS=40 (scaled: val/1000 = dB/KM) */

  int16_t threshold_param;	 /* Overlaied threshold in .1; Range 0 - 200;
  ...
  int16_t spot_blank_flag;	 /* BDSPTBLNK spot blank flag: 0 - none; 
				  * 1 - for radial; 2 - for elevation; 
				  * 4 - for volume */

  int16_t word_43;		 /* unused 2 bytes */
  int16_t word_44;		 /* unused 2 bytes */

  float fazimuth;		/* Temporary storage for floating point 
                           	   azimuth (used to support Message 31). 4 bytes */

  float felevation;		/* Temporary storage for floating point 
                            	   elevation (used to support Message 31). 4 bytes */

  int16_t word_49;		 /* unused 2 bytes */
  int16_t word_50;		 /* unused 2 bytes */
  int16_t word_51;		 /* unused 2 bytes */
  int16_t word_52;		 /* unused 2 bytes */
  int16_t word_53;		 /* unused 2 bytes */
  int16_t word_54;		 /* unused 2 bytes */
  int16_t word_55;		 /* unused 2 bytes */
  int16_t word_56;		 /* unused 2 bytes */
  int16_t word_57;		 /* unused 2 bytes */
  int16_t word_58;       /* unused 2 bytes */
...
typedef struct orda_nx_msg_hdr {
  ...
  int16_t elev_num;       	/* RDA elevation number within volume (1, 2, ...)
			   	   scan BDENUM=17 */
  ...
  int16_t word_34;		/* unused */
  int16_t word_35;		/* unused */
  int16_t word_36;		/* unused */
  int16_t word_37;		/* unused */
  int16_t word_38;		/* unused */

  int16_t nyquist_vel;		/* Nyquist velocity BDNQVEL=39 
			   	   (scaled: val/100 = m/s) 
  ...
  int16_t spot_blank_flag;	/* BDSPTBLNK spot blank flag: 0 - non; 
			   	   1 - for radial; 2 - for elevation; 
			   	   4 - for volume */

  int16_t word_43;		/* unused */
  int16_t word_44;		/* unused */
  ...
  int16_t word_49;		/* unused */
  int16_t word_50;		/* unused */
  int16_t word_51;        	/* unused */
  int16_t word_52;        	/* unused */
  int16_t word_53;        	/* unused */
  int16_t word_54;        	/* unused */
  int16_t word_55;        	/* unused */
  int16_t word_56;        	/* unused */
  int16_t word_57;        	/* unused */
  int16_t word_58;        	/* unused */
...
typedef struct {
    ...
    int16_t msg_type;		/* bit flags: (bit 0 LSB)
				   ...
				   10-th bit: Super Resolution Recombined cut
                                   11-th bit: Preprocessed Dual-Pol cut                                            13-th bit: High Attenuation Radial */


    int16_t version;              /* Version number for radial format. */
    ...
    uint16_t date;	/* Radial date, modified Julian. */

    uint16_t begin_vol_date; /* Volume start date, modified Julian */

    float latitude;             /* Latitude of RDA. */

    float longitude;            /* Longitude of RDA. */

    uint16_t height;      /* Height of radar, in meters MSL. */

    uint16_t feedhorn_height; /* Height of feedhorn, in meters MSL. */  

    int16_t weather_mode;		/* 1 (clear air) or 2 (convective) */

    int16_t vcp_num;		/* Volume coverage pattern. */

    int16_t volume_scan_num;	/* Volume scan number, 1 - 80. */   
                                /* Recycles to 1 after 80. */

    int16_t vol_num_quotient;	/* Quotient for dividing volume sequence number
				   by MAX_VSCAN. */
    ...
    int16_t azi_num;		/* Radial number within elevation sweep. */

    int16_t elev_num;		/* RDA elevation number within VCP. */

    int16_t rpg_elev_ind;		/* RPG elevation index corresponding to 
                                   RDA elevation number. */

    int16_t target_elev;		/*  Target elevation, in .1 degrees.  From VCP
                                    definition. */

    int16_t last_ele_flag;	/* = 1 if this is the last cut 
				   = 0 otherwise. */

    int16_t start_angle;		/* Radial start azimuth, in .1 degrees. */

    int16_t delta_angle;		/* Radial width, in .1 degrees. */
    ...
    int16_t status;		/* Radial status.  Psuedo end of volume
    ...
    int16_t atmos_atten;		/* Atmospheric attenuation, in dB/km*1000. */

    int16_t spot_blank_flag;	/* Spot blanking bit map. */
    ...
    int16_t sector_num;           /* Doppler PRF Sector Number. */

    int16_t vel_offset;		/* Byte offset to start of velocity data. 
                                   The velocity data must be word aligned. */

    int16_t n_dop_bins;		/* Number of Doppler bins. */

    int16_t dop_bin_size;		/* Doppler bin size in meters. */

    int16_t dop_range;		/* Range to start of Doppler data, 
                                   in number of bins. */

    int16_t range_beg_dop;        /* Range to beginning of 1st Doppler bin, 
                                   in meters. */

    int16_t dop_resolution;	/* = 1 if vel_resolution = 2 (.5 m/s); 
				   = 2 otherwise. */

    int16_t unamb_range;		/* Unambiguous range. */

    int16_t nyquist_vel;		/* Nyquist velocity. */

    int16_t vel_snr_thresh;       /* SNR applied to velocity, in dB*8. */

    int16_t vel_tover;            /* Tover applied to velocity. */

    int16_t ref_offset;		/* Byte offset to start of reflectivity data. 
                                   The reflectivity data must be word aligned. */

    int16_t n_surv_bins;		/* Number of surveilance bins in the msg */;

    int16_t surv_bin_size;	/* Bin size in meters. */

    int16_t surv_range;		/* Range to start of surveillance data,
                                   in number of bins. */

    int16_t range_beg_surv;       /* Range to beginning of 1st surveillance bin, 
                                   in meters. */

    int16_t sc_azi_num;           /* Split cut azimuth number of reflectivity 
                                   radial. */

    int16_t surv_snr_thresh;      /* SNR applied to reflectivity moment, in dB*8. */

    int16_t spw_offset;		/* Byte offset to start of spectrum width data.  
                                   The spectrum width data must be word aligned. */

    int16_t spw_snr_thresh;       /* SNR applied to spectrum width, in dB*8. */

    int16_t spw_tover;            /* Tover applied to spectrum width. */

    int16_t spare3;               /* Ununsed. */

    int16_t spare4;               /* Ununsed. */

    int16_t no_moments;           /* Number of offsets to additional moments
                                   that follow. */
...
typedef struct {
   ...
   int16_t accunrng[MAX_ELEVS][MAX_SECTS];  /* Unambiguous range, in
                                             km*10. */
 
   int16_t accunvel[MAX_ELEVS][MAX_SECTS];  /* Unambiguous velocity,
                                             in m/s*100. */
   ...
   uint32_t acceltms[MAX_ELEVS];   /* Starting elevation time, in
                                  milliseconds. */

   uint32_t acceltme[MAX_ELEVS];   /* Ending elevation time, in
                                  milliseconds. */
   ...
   int16_t accnumrd[MAX_ELEVS];  /* Number of radials this 
                                  elevation. */

   int16_t accatmos[MAX_ELEVS];  /* Atmospheric attenuation, in
                                  dB/km*1000. */

   /* The following items are considered constant during the volume
      scan. */

   int16_t accnumel;             /* Number of elevation cuts 
                                  this volume scan. */

   int16_t accthrparm;           /* Threshold parameter TOVER. */

   int16_t accwxmode;            /* Weather Mode. */

   int16_t accvcpnum;            /* Volume Coverage Pattern Number. */

   int16_t accdopres;            /* Doppler data resolution. */

   float acccalib;             /* Calibration constant. */

   int16_t accvsdate;             /* Volume scan start date, modified
                                  Julian. */
...
typedef struct nx_vol_title {
  ...
  uint32_t julian_date;              /* Modified julian date referenced from 1/1/70 */
  uint32_t millisecs_past_midnight;  /* Time - Millisecs of day from midnight
                                  * (GMT) when file was created */
  uint32_t filler1;                  /* unused */
} NEXRAD_vol_title;

typedef struct nx_ctm_info {
  int16_t word1;  /* This stuff is to be ignored.  It is used for checking */
  int16_t word2;  /* data integrity during the transmission process */
  int16_t word3;
  int16_t word4;
  int16_t word5;
  int16_t word6;
